...
/*
Changes: 
5: changed comparisons "==" to "===" - FredHappyface
*/
...
for (var index = 0; index < IDS_LEN; index++){
	Block.registerDropFunctionForID(IDS[index], function(coords, id, data, level, enchant){
		if(level > 0){
			return [[id, 1, data%8]];
		}
		return [];
...
var SILK_IDS = [79, 174]
var SILK_IDS_LEN = SILK_IDS.length
for (var index = 0; index < SILK_IDS_LEN; index++){
	Block.registerDropFunctionForID(SILK_IDS[index], function(coords, id, data, level, enchant){
